...
public class FantasyLeagueAutoDraftRankingEngine {
    ...
    static class Player implements Comparable<Player> {
        ...
        private double getFantasyPoints() {
            return matchesPlayed * 2.0 + battingAverage;
        }
    ...
    static String draftAndRank(Player[] players) {
        ...
        Player[] shortlisted = new Player[players.length];
        int count = 0;
        ...
        for (Player player : players) {
            ...
            if (Player.isDraftable(player.matchesPlayed)
                ...
                shortlisted[count] = player;
                count++;
            }
        }

        Player[] finalList = Arrays.copyOf(shortlisted, count);
